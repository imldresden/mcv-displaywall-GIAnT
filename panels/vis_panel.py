...
class VisPanel(avg.DivNode):
    ...
    def __init__(self, label, vis_params, axis_size, show_grid, aspect=None, parent=None, **kwargs):
        ...
        avg.RectNode(pos=(self.__axis_size.x, 0), size=data_div_size,
                strokewidth=0, fillopacity=1, fillcolor=global_values.VIS_PANEL_BACKGROUND, parent=self)
...
class AxisNode(avg.DivNode):
    ...
    def __init__(self, data_range, unit="m", hide_rims=False, top_axis=False, inverted=False, parent=None,
                 label_offset=0, tick_positions=None, tick_labels=None, label_fontsize=global_values.FONT_SIZE, **kwargs):
        """
        ...
        if tick_labels is None:
            ...
            if len(tick_labels) < len(tick_positions):
                raise Exception("The length of the tick_positions isn't allowed to be smaller than the length of the tick_labels!")
    ...
    @tick_labels.setter
    def tick_labels(self, value):
        self.__tick_labels = value
        if len(self.__tick_labels) < len(self.__tick_positions):
            raise Exception("The length of the tick_positions isn't allowed to be smaller than the length of the tick_labels!")

        for i, label_node in enumerate(self.__label_nodes):
            label_node.text = self.__format_label(self.__tick_positions[i])
    ...
    def __draw_ticks(self):
        ...
        for i, pos in enumerate(self.__label_pos):
            ...
            self.__label_nodes[i] = libavg.WordsNode(color=global_values.COLOR_FOREGROUND,
                    fontsize=self.__label_fontsize, parent=self)
